...
if ('authentication_status' in st.session_state) and (st.session_state['authentication_status'] == True) and ('username' in st.session_state) and ('email' in st.session_state):
    ...
    # if email in list(client_data['email']) and username in list(client_data['username']) and ('Edit' not in st.session_state or st.session_state['Edit'] == 0):
    #     show_client_data()
    ...
    #     username = st.session_state['username']
    #     gather_client_data()
    if email in list(client_data['email']) and username in list(client_data['username']):
        if 'Edit' not in st.session_state:
            st.session_state['Edit'] = 0

        if st.session_state['Edit'] == 1:
            gather_client_data()
        else:
            show_client_data()
    else:
