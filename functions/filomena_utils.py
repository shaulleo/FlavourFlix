...
def get_identification_and_user():
    """Returns the identification of the user, if it exists.
    Parameters:
        - None
    Returns:
        - identification (str): Identification of the user.
        """
    #If the user is correctly logged on
    if ('authentication_status' in st.session_state) and (st.session_state['authentication_status'] == True) and ('username' in st.session_state):
        #Extract username and first name
...
def get_profile():
    """Returns the profile of the user, if it exists.
    Parameters:
        - None
    Returns:
...
def get_data_match(data: pd.DataFrame, word:str, col_to_match: str, method: str='dot'):
    """Returns the closest match of a word in a column of a dataframe.
    Parameters:
        - data (pandas.DataFrame): DataFrame containing the data.
    Returns:
        - data_match (str): Closest match of the word in the column of the dataframe."""
    
    #Load embeddings
    nlp = spacy.load("en_core_web_md")
    #Find if there is a direct match in the data
    data_match = data[data[col_to_match].str.contains(word)].head(1)
    #If there is no data match
    if len(data_match) == 0:
        #Handle accents
        ...
        similarities = {}
        #For each possible value in the col to match
        for possible_match in list(data[col_to_match].unique()):
            ...
            #If the word is in the possible match
            if word_clean in possible_match_clean:
                ...
                #Create the embedding of word and the possible match
                ...
                #Calculate the similarity between the word and the possible match either through the dot product or cosine similarity
                if method == 'dot':
...
def filter_schedule(restaurants: pd.DataFrame, time_slot: str=None):
    """Returns the restaurants that are open in a given time slot.
    Parameters:
        - restaurants (pandas.DataFrame): DataFrame containing the restaurants.
        - time_slot (str): Time slot in which the restaurants should be open.
    Returns:
        - filtered_restaurants (pandas.DataFrame): DataFrame containing the restaurants that are open in the given time slot."""
    
    def contains_time_interval(schedule):
        """Returns whether a restaurant is open in a given time slot.
        Parameters:
            - schedule (str): Schedule of the restaurant.
        Returns:
            - open (bool): Whether the restaurant is open in the given time slot."""
        if schedule in ['Closed', 'Not Available']:
            return False
        schedule_dict = {}
        #If schedule available get the dictionary
        try:
        ...
        for day_schedule in schedule_dict.values():
            ...
            for time in times:
                ...
                #If the start or the end of the time slot is between the start and end of the restaurant schedule
                #return True
    ...
    if time_slot is None:
        return restaurants
    #If there is effectively a time slot, find the start and the end of the time slot
    else:
        time_start, time_end = map(pd.to_datetime, time_slot.split(' - '))

        filtered_restaurants = []
        #Find the restaurants that are open during the provided time slot
...
def get_personality(username: str):
    """Returns the personality of the user, if it exists in the questionnaire database.
    Parameters:
        - username (str): Username of the user.
    Returns:
        - personality (str): Personality of the user."""
    #Import the data
    personality_questionnaire = pd.read_csv('data/training_answers/perturbed_total_answers.csv')
    #If the user is in the database
    if username in personality_questionnaire['username'].values:
        #Return their personality
        return personality_questionnaire[personality_questionnaire['username'] == username]['personality'].values[0]
    else:
        return 'Not Available'
    

def get_preferences(personality: str):
    """Returns the preferences of the user, if it exists in the questionnaire database.
    Parameters:
        - personality (str): Personality of the user.
    Returns:
        - preferences (str): Preferences of the user.
    """
    #Define the questions and parameter names
    ...
    #If the personality is available
...
#Store the questionnaire questions and identifiers
...
#Instructions for the core piece of Filomena
instructions = {
...
#Prompts to identify the type of instruction
...
#Prompts for the question answering task with document retrieval and memory
...
#Prompts for the restaurant description task
...
#Prompts for the restaurant recommendation task
...
#Prompts for the personality identification task                                                      
...
#Prompts for the dissatisfcation task
